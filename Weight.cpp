...
Weight::Weight() noexcept
		:Weight(UNKNOWN_WEIGHT, DEFAULT_UNIT_OF_WEIGHT, UNKNOWN_WEIGHT)
{ }
...
Weight::Weight(t_weight newWeight, Weight::UnitOfWeight newUnitOfWeight) noexcept
		:Weight(newWeight, newUnitOfWeight, UNKNOWN_WEIGHT)
{ }

// ↓ #5  newWeight and newMaxWeight provided: delegates to #7 with default value for _unitOfWeight
Weight::Weight(t_weight newWeight, t_weight newMaxWeight) noexcept
		:Weight(newWeight, DEFAULT_UNIT_OF_WEIGHT, newMaxWeight)
{ }

// ↓ #6  newUnitOfWeight and newMaxWeight provided: delegates to #7 with default value for _weight
Weight::Weight(UnitOfWeight newUnitOfWeight, t_weight newMaxWeight) noexcept
		:Weight(UNKNOWN_WEIGHT, newUnitOfWeight, newMaxWeight)
{ }

// ↓ #7  all member variables provided as parameters
Weight::Weight(t_weight newWeight, Weight::UnitOfWeight newUnitOfWeight, t_weight newMaxWeight) noexcept
		:_unitOfWeight{newUnitOfWeight}
{
...
t_weight Weight::convertWeight(const t_weight fromWeight, const Weight::UnitOfWeight fromUnit, const Weight::UnitOfWeight toUnit) noexcept
{
	...
	case POUNDS:
		switch (toUnit)
		{
		case POUNDS: return fromWeight;
		case SLUGS: return fromPoundsToSlugs(fromWeight);
		case KILOS: return fromPoundsToKilograms(fromWeight);
		default: assert(0);
		}
	...
	case SLUGS:
		switch (toUnit)
		{
		case POUNDS: return fromSlugsToPounds(fromWeight);
		case SLUGS: return fromWeight;
		case KILOS: return fromPoundsToKilograms(fromSlugsToPounds(fromWeight));
		default: assert(0);
		}
	...
	case KILOS:
		switch (toUnit)
		{
		case POUNDS: return fromKilogramsToPounds(fromWeight);
		case SLUGS: return fromPoundsToSlugs(fromKilogramsToPounds(fromWeight));
		case KILOS: return fromWeight;
		default: assert(0);
		}
	default: assert(0);
	}
...
t_weight Weight::getWeight() const noexcept
{
	...
	assert(0); //invalid weight encountered, force crash
}
...
std::ostream& operator<<(ostream& lhs_stream, const Weight::UnitOfWeight rhs_UnitOfWeight)
{
	...
	{
	case Weight::POUNDS: return lhs_stream << Weight::POUND_LITERAL;
	case Weight::KILOS: return lhs_stream << Weight::KILO_LITERAL;
	case Weight::SLUGS: return lhs_stream << Weight::SLUG_LITERAL;
	default: throw out_of_range(PROGRAM_NAME " Weight Error: The unit can not be converted to a string!");
	}
}
